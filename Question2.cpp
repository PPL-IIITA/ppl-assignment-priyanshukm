...
int main()
{
	...
	for(int i = 0, cur = 0; i < couplehappiness.size() && cur < k; i++, cur++ )
	{
		...
		cout<<boyslist[index].name<<" "<<((boyslist[index].gf)->name)<<" "<<couplehappiness[i].first<<endl;
	}

	cout<<k<<" Most Compatible Couples : \n";
	for(int i = 0, cur = 0; i < couplecompatibility.size() && cur < k; i++, cur++ )
	{
		int index = couplecompatibility[i].second;
		cout<<boyslist[index].name<<" "<<((boyslist[index].gf)->name)<<" "<<couplecompatibility[i].first<<endl;
	}
